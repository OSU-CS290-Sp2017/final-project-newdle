...
function acceptSignupNewdleModal(){
	...
	if(signupName != ""){
		if(timesArray.length > 0){
			saveTimes(signupName, signupNewdle, timesArray);
		}
		else{
...
function saveTimes(name, timesToSave) {
	...
	var jsonObject = {"accountNum" : currentAccountJSON.accountNum,
...
function closeSignupNewdleModal() {
	...
	editNewdleModal.querySelector('#input-signup-name').value = "";
}
